...
import { Question, QuestionType, GradeLevel, Difficulty, QuestionCategory, QuestionFilters, DEFAULT_CONTENT_CATEGORIES } from '../types';
...
export const mapQuestionFromDB = (q: any): Question => ({
  id: q.id,
  type: q.type as QuestionType,
  text: q.text,
  imageUrls: q.image_urls || [],
  options: q.options || undefined,
  correctAnswer: q.correct_answer || '', // Handle missing correct_answer for secure view
  subject: q.subject,
  gradeLevel: q.grade_level as GradeLevel,
  difficulty: q.difficulty as Difficulty,
  category: (q.category as QuestionCategory) || QuestionCategory.BASIC,
  contentCategory: q.content_category || DEFAULT_CONTENT_CATEGORIES[DEFAULT_CONTENT_CATEGORIES.length - 1],
...
export const getQuestions = async (
  ...
  let query = supabase
    .from('questions')
    .select('id, type, text, correct_answer, subject, grade_level, difficulty, category, content_category, created_at, is_disabled, is_deleted, score', { count: 'exact' });
...
export const getQuestionsByIds = async (ids: string[], includeDeleted: boolean = false, secureMode: boolean = false): Promise<Question[]> => {
    ...
    const fetchChunk = async (chunk: string[], retries = 3): Promise<any[]> => {
        try {
            // Use secure view if requested (for Quiz Taking) or default to questions table (for Admin)
            const tableName = secureMode ? 'questions_safe_view' : 'questions';
            let query = supabase.from(tableName).select('*').in('id', chunk);
            ...
            // Fallback logic: If secureMode is FALSE but we get a permission error (403),
            // it means a non-admin is trying to access the questions table.
            // We should fallback to the secure view automatically.
            if (error && (error.code === '42501' || error.message?.includes('permission'))) {
                 logger.warn('DB', 'Permission denied on questions table, falling back to secure view', { tableName });
                 const fallbackQuery = supabase.from('questions_safe_view').select('*').in('id', chunk).neq('is_deleted', true);
...
export const updateQuestionsContentCategory = async (fromCategory: string, toCategory: string): Promise<void> => {
    if (!fromCategory || !toCategory || fromCategory === toCategory) return;
    const { error } = await supabase
        .from('questions')
        .update({ content_category: toCategory })
        .eq('content_category', fromCategory);
    if (error) {
        logger.error('DB', 'Error updating content category for questions', { fromCategory, toCategory }, error);
        throw error;
    }
    logger.info('DB', 'Updated content category for questions', { fromCategory, toCategory });
};

export const updateQuestionsContentCategoryForEmpty = async (toCategory: string): Promise<void> => {
    if (!toCategory) return;
    const { error } = await supabase
        .from('questions')
        .update({ content_category: toCategory })
        .or('content_category.is.null,content_category.eq.');
    if (error) {
        logger.error('DB', 'Error updating empty content category for questions', { toCategory }, error);
        throw error;
    }
    logger.info('DB', 'Updated empty content category for questions', { toCategory });
};
...
export const saveQuestion = async (question: Question): Promise<Question> => {
  ...
  const dbPayload = {
    ...
    content_category: rest.contentCategory || DEFAULT_CONTENT_CATEGORIES[DEFAULT_CONTENT_CATEGORIES.length - 1],
...
export const updateQuestion = async (updatedQuestion: Question): Promise<void> => {
  ...
  const dbPayload = {
    ...
    content_category: updatedQuestion.contentCategory || DEFAULT_CONTENT_CATEGORIES[DEFAULT_CONTENT_CATEGORIES.length - 1],
  ...
  try {
      ...
      if (oldQuestion) {
          ...
          if (oldQuestion.contentCategory !== updatedQuestion.contentCategory) diff['contentCategory'] = { old: oldQuestion.contentCategory, new: updatedQuestion.contentCategory };
...
export const getAvailableQuestionCount = async (
    ...
    if (contentCategories.length > 0) query = query.in('content_category', contentCategories);
