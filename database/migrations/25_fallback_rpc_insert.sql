-- 25_fallback_rpc_insert.sql

-- ==========================================
-- 1. Helper Functions (Fix Search Path)
-- ==========================================
-- Recreate helpers with proper search_path including 'extensions'
-- This ensures they can find any extension functions if needed.

CREATE OR REPLACE FUNCTION public.arrays_equal_unordered(json_a JSONB, json_b JSONB) 
RETURNS BOOLEAN IMMUTABLE LANGUAGE plpgsql SECURITY DEFINER 
...
-- ==========================================
-- 2. Submit Quiz RPC (Debug & Fix)
-- ==========================================
-- Wrap in exception block to catch and log errors.
...
DECLARE
  ...
  v_user_parts TEXT[];
BEGIN
  -- Set a session variable to flag this as a trusted execution
  PERFORM set_config('app.is_secure_context', 'true', true);
  ...
  INSERT INTO public.quiz_results (
    ...
    gen_random_uuid(), -- Requires pgcrypto (built-in in PG13+)
    v_user_id, 
    v_username,
...
-- ==========================================
-- 3. System Logs RLS (Fix Already Exists Error)
-- ==========================================
...
-- ==========================================
-- 4. Quiz Results RLS & Trigger (Fix Insert Issue)
-- ==========================================
...
-- Create Trigger to block direct inserts (Bypass only if app.is_secure_context is true)
CREATE OR REPLACE FUNCTION public.check_secure_context()
RETURNS TRIGGER AS $$
BEGIN
  -- Allow if Superuser (Admin) or if secure context flag is set
  IF public.is_super_admin() OR current_setting('app.is_secure_context', true) = 'true' THEN
     RETURN NEW;
  END IF;
