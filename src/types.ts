...
export interface QuizAttempt {
  ...
  needsGrading?: boolean; // Alternative field name from backend
}
...
export interface QuizPartConfig {
  ...
  // subjects: string[];  // Moved to QuizConfig
  difficulties: Difficulty[];
  // gradeLevels: GradeLevel[]; // Moved to QuizConfig
...
export const DEFAULT_CONTENT_CATEGORIES = ['课后习题知识性', '小学配题知识性', '初中配题知识性', '匹配性', '默认'];
...
export interface QuizConfig {
  ...
  // Global Filters (Moved from QuizPartConfig)
  subjects?: string[];
  gradeLevels?: GradeLevel[];
  contentCategories?: string[]; // New global filter
  ...
  allowOneAttempt?: boolean; // New field: Allow only one attempt per user
  isDeleted?: boolean; // New field for soft delete status
  duration?: number; // Quiz duration limit in minutes (0 = unlimited)
  lastResetAt?: number; // Timestamp of last attempt reset
}
