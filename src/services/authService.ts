  ...
  if (signUpData.user) {
      ...
      if (profileError) {
          ...
          if (profileError.code === '23505') {
              logger.info('AUTH', 'Profile already created by system trigger, updating with additional fields', { username, userId: signUpData.user.id });
              
              // Update the existing profile with the additional fields
              ...
              if (updateError) {
                  logger.error('AUTH', 'Failed to update profile fields after trigger creation', { username, userId: signUpData.user.id }, updateError);
              }
          } else {
...
export const adminAddUser = async (
    ...
    const { data, error } = await supabase.rpc('admin_create_user', {
        ...
        new_role: role,
        new_provider_name: providerName || null,
        new_school_stage: schoolStage || [],
        new_subject: subject || []
    });
...
export const deleteUser = async (userId: string): Promise<{ success: boolean; error?: any }> => {
    // Note: Parameter name updated to target_user_id to match DB function signature
...
export const updateUserProfile = async (userId: string, updates: any): Promise<{ success: boolean; error?: any }> => {
    ...
    const { error } = await supabase.from('profiles').update({
        ...
        provider_name: profileUpdates.providerName,
        school_stage: profileUpdates.schoolStage,
        subject: profileUpdates.subject
    }).eq('id', userId);
