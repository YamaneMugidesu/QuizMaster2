-- 34_debug_and_fix_rpc.sql

-- ==========================================
-- Fix submit_quiz RPC (Robust JSON Parsing & Debugging)
-- ==========================================
-- 1. Uses jsonb_array_elements instead of jsonb_to_recordset to avoid key mapping ambiguity.
-- 2. Adds safety check to ensure input questions actually match DB questions.
-- 3. Fixes potential JSON casting error for empty answers in Multiple Select.
...
BEGIN
  ...
  -- Grading Query (Robust Set-based approach)
  ...
  -- SAFETY CHECK: If we had inputs but got no attempts, something is wrong with the Join or Parsing
  IF v_input_count > 0 AND jsonb_array_length(v_attempts) = 0 THEN
      -- Log the failure details for debugging
      RAISE EXCEPTION 'Grading Error: Submitted % answers but matched 0 questions. Please check if questions exist or are deleted.', v_input_count;
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  -- Try to log error if table exists, otherwise just raise
  BEGIN
    INSERT INTO public.system_logs (
        ...
        'RPC_SUBMIT_QUIZ',
        SQLERRM,
        jsonb_build_object('state', SQLSTATE, 'config_id', p_config_id, 'inputs_count', jsonb_array_length(p_answers)),
        auth.uid(),
        extract(epoch from now()) * 1000
    );
  EXCEPTION WHEN OTHERS THEN
    -- If logging fails, just ignore it and raise the original error
    NULL;
  END;
  RAISE;
END;
