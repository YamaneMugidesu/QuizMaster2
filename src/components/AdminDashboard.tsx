...
import { Question, QuestionType, Difficulty, GradeLevel, QuestionFormData, UserRole, QuizResult, User, SUBJECTS, QuizConfig, QuestionCategory, DEFAULT_CONTENT_CATEGORIES } from '../types';
import { saveQuestion, updateQuestion, deleteQuestion, toggleQuestionVisibility, deleteQuizResult, restoreQuestion, hardDeleteQuestion, getQuestionById, getResultById, getSystemSetting } from '../services/storageService';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, onViewResult }) => {
  ...
  useEffect(() => {
      const timer = setTimeout(() => {
          setDebouncedFilters({
              ...
              contentCategory: filterContentCategory
          });
      }, 300);
      return () => clearTimeout(timer);
  }, [searchTerm, filterSubject, filterGrade, filterType, filterDifficulty, filterCategory, filterContentCategory]);
  ...
  const { 
      ...
      contentCategory: debouncedFilters.contentCategory,
  ...
  const [debouncedRecordFilters, setDebouncedRecordFilters] = useState(recordFilters);

  // Debounce Record Filters
  useEffect(() => {
      const timer = setTimeout(() => setDebouncedRecordFilters(recordFilters), 300);
      return () => clearTimeout(timer);
  }, [recordFilters]);
  ...
  } = useAllResults(currentPage, itemsPerPage, debouncedRecordFilters, activeTab === 'records');
  ...
  const getGradeLabel = (g: GradeLevel) => {
      switch(g) {
          ...
          case GradeLevel.COMPREHENSIVE: return '综合';
          default: return '小学';
      }
  }
  ...
  const getCategoryColor = (c?: QuestionCategory) => {
      switch(c) {
          ...
          case QuestionCategory.MOCK_AUDIT: return 'bg-purple-100 text-purple-800 border-purple-200';
          case QuestionCategory.DEFAULT: return 'bg-gray-100 text-gray-800 border-gray-200';
      ...
      {activeTab === 'create' && (
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 animate-fade-in">
           <QuestionForm 
             ...
             showContentCategoryManager={currentUser.role === UserRole.SUPER_ADMIN}
           />
        </div>
      )}
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            {showRecycleBin ? "退出回收站" : "回收站"}
                        </button>
                        {(searchTerm || filterSubject || filterGrade || filterType || filterDifficulty || filterCategory || filterContentCategory || showRecycleBin) && !showRecycleBin && (
                            <button 
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    <select 
                        className="block w-full pl-3 pr-8 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-lg"
                        value={filterSubject}
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-sm text-gray-500">
                    <span className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-900">{totalQuestions}</span> 个{showRecycleBin ? '已删除题目' : '符合条件的题目'}
                    </span>
                </div>
            </div>
         ...
         <div className="space-y-6 animate-fade-in">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                     <div>
                        <input
                            type="text"
                            placeholder="搜索用户名..."
                            value={recordFilters.username}
                            onChange={(e) => setRecordFilters({...recordFilters, username: e.target.value})}
                     ...
                     <div>
                        <input
                            type="text"
                            placeholder="搜索试卷名称..."
                            value={recordFilters.configName}
                            onChange={(e) => setRecordFilters({...recordFilters, configName: e.target.value})}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-sm"
                        />
                     </div>
                     <div>
                        <select 
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-sm bg-white"
                            value={recordFilters.contentCategory}
                            onChange={(e) => setRecordFilters({...recordFilters, contentCategory: e.target.value})}
                        >
                            <option value="">所有内容分类</option>
                            {contentCategoryOptions.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                     ...
                     <div>
                        ...
                        >
                            ...
                            <option value="pending_grading">待批改</option>
                        </select>
                     </div>
                 </div>
                 
                 {(recordFilters.username || recordFilters.supplier || recordFilters.configName || recordFilters.contentCategory || recordFilters.status) && (
                     <div className="mt-4 flex justify-between items-center text-sm">
                         <span className="text-gray-500">
                             共找到 <span className="font-semibold text-gray-900">{totalUserResults}</span> 条记录
                         </span>
                         <button 
                            onClick={() => setRecordFilters({
                                username: '',
                                supplier: '',
                                configName: '',
                                contentCategory: '',
                            ...
                            className="text-primary-600 hover:text-primary-800 font-medium bg-primary-50 px-3 py-1 rounded-md transition-colors"
                        >
                            重置筛选
                        </button>
                     </div>
             ...
             {isResultsLoading ? (
                ...
                <div className="p-16 text-center text-gray-500 flex flex-col items-center justify-center">
                   ...
                   {(recordFilters.username || recordFilters.supplier || recordFilters.configName || recordFilters.contentCategory || recordFilters.status) ? '未找到符合条件的答题记录' : '暂无任何用户答题记录'}
                </div>
             ) : (
                   ...
                   <table className="w-full text-left">
                      <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                        <tr>
                          ...
                          <th className="px-6 py-4 font-semibold">试卷名称</th>
                                 ...
                                 <div className="flex items-center justify-end gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                     {result.status === 'pending_grading' ? (
                                         <button 
                                             ...
                                             className="text-white bg-orange-500 hover:bg-orange-600 px-3 py-1.5 rounded text-xs shadow-sm transition-colors flex items-center gap-1"
                                         >
                                             <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                             {processingId === result.id ? '...' : '去批改'}
                                         </button>
                                     ) : (
                                         <button 
                                            onClick={(e) => handleViewResultDetail(e, result)} 
                                            className="text-primary-600 hover:text-primary-800 hover:bg-primary-50 px-2 py-1 rounded transition-colors"
                                         >
                                             查看详情
                                         </button>
                                     )}
