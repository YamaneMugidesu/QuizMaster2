...
-- 41_fix_submit_quiz_json_grading.sql

-- ==========================================
-- Fix RPC grading logic for JSON-formatted Fill-in-the-blank answers
-- ==========================================
-- The 'correct_answer' for FILL_IN_THE_BLANK questions is now stored as a JSON array (e.g. '["teeth"]').
-- The previous logic used string_to_array(..., ';&&;'), which fails to parse JSON strings correctly 
-- (it treats '["teeth"]' as a single string including brackets and quotes).
-- This update adds logic to detect and parse JSON answers.
      ...
      CASE 
        ...
        WHEN q.type = 'FILL_IN_THE_BLANK' THEN
           (
             ...
             FROM unnest(
                string_to_array(p.u_answer, ';&&;'), 
                CASE 
                    -- Fix: Check if correct_answer is a JSON array (starts with [)
                    -- If so, parse it as JSONB and expand to text array
