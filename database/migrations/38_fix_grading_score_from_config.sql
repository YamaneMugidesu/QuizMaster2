-- 38_fix_grading_score_from_config.sql

-- ==========================================
-- Fix RPC to respect Quiz Config Scores
-- ==========================================
-- Previously, the RPC used the default score from the `questions` table.
-- Now, it will correctly lookup the score defined in the `quiz_configs.parts` 
-- based on the question type and difficulty, matching the frontend logic.
...
BEGIN
  ...
  VALUES (gen_random_uuid(), 'INFO', 'DEBUG_RPC', 'submit_quiz_graded', 
    jsonb_build_object('total_score', v_total_score, 'attempts_count', jsonb_array_length(v_attempts)),
    v_user_id, extract(epoch from now()) * 1000);
