...
export const QuizResultView: React.FC<QuizResultProps> = ({ result, onRetry, onExit, isAdmin = false }) => {
  ...
  useEffect(() => {
      const load = async () => {
          ...
          // Fetch full question details for review (including text and images)
          if (currentResult.attempts && currentResult.attempts.length > 0) {
              const missingQuestionIds = currentResult.attempts
                  .map(a => a.questionId)
                  .filter(id => !fetchedQuestions.has(id));
              
              if (missingQuestionIds.length > 0) {
                  try {
                      // Fix: Use secureMode=true to allow standard users to fetch questions from safe view
                      // Standard users do not have access to 'questions' table directly.
                      const questions = await getQuestionsByIds(missingQuestionIds, true, true);
                      
                      const newMap = new Map(fetchedQuestions);
                      questions.forEach(q => newMap.set(q.id, q));
                      setFetchedQuestions(newMap);
                  } catch (e) {
                      console.error("Failed to fetch question details for review", e);
                  }
              }
          }
  ...
  useEffect(() => {
      ...
      const fetchPageQuestions = async () => {
          ...
          if (uniqueIds.length > 0) {
              try {
                  // Fix: Use secureMode=true for standard users
                  const newQuestions = await getQuestionsByIds(uniqueIds, true, true);
  ...
  // Disable Retry Button if "Allow One Attempt" is enabled and not admin
  const isRetryDisabled = !isAdmin && config?.allowOneAttempt;
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 text-center p-10">
        ...
        <div className="flex justify-center gap-4">
            ...
            {onRetry && <Button onClick={onRetry} disabled={isRetryDisabled} title={isRetryDisabled ? "该试卷仅允许作答一次" : "重新开始"}>再测一次</Button>}
        </div>
      </div>
      ...
      {config && (isAdmin || config.quizMode !== 'exam') && (
      <div 
        id="review-section" 
                             ...
                             <div className="flex items-start gap-4">
                                 ...
                                 <div className="flex-1">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                            ...
                                            <>
                                                <div className={`p-3 rounded-lg border ${attempt.isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                                                    ...
                                                    <div className="font-semibold break-words">
                                                        {qObj?.type === QuestionType.FILL_IN_THE_BLANK ? (
                                                            <div className="flex flex-col gap-1">
                                                                {attempt.userAnswer?.split(';&&;').map((ans: string, idx: number) => (
                                                                    <div key={idx} className="flex gap-2 items-start">
                                                                        <span className="text-gray-500 w-4 flex-shrink-0 mt-0.5">{idx + 1}.</span>
                                                                        <div className="flex-1 min-w-0">
                                                                            <RichTextPreview content={ans} placeholder="(未作答)" />
                                                                        </div>
                                                                    </div>
                                                                )) || "(未作答)"}
                                                            </div>
                                                            ...
                                                            {qObj?.type === QuestionType.FILL_IN_THE_BLANK ? (
                                                                <div className="flex flex-col gap-1">
                                                                    {(() => {
                                                                        ...
                                                                        try {
                                                                            if (correctTextRaw?.startsWith('[')) {
                                                                                const parsed = JSON.parse(correctTextRaw);
                                                                                if (Array.isArray(parsed)) answers = parsed;
                                                                            }
                                                                        } catch {}
                                                                        
                                                                        // Fallback to split (legacy format)
                                                                        if (answers.length === 0 && correctTextRaw) {
                                                                            answers = correctTextRaw.split(';&&;');
                                                                        }
                                                                        ...
                                                                        return answers.map((ans: string, idx: number) => (
                                                                            <div key={idx} className="flex gap-2 items-start">
                                                                                <span className="text-gray-500 w-4 flex-shrink-0 mt-0.5">{idx + 1}.</span>
                                                                                <div className="flex-1 min-w-0">
                                                                                    <RichTextPreview content={ans} placeholder="(未知)" />
                                                                                </div>
                                                                            </div>
                                                                        ));
                                                                    })() || "(未知)"}
                                     ...
                                     <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100 text-blue-800">
                                         <div className="flex flex-col sm:flex-row gap-1">
                                             <span className="font-bold mr-1 flex-shrink-0">💡 解析:</span>
                                             <div className="flex-1 min-w-0">
