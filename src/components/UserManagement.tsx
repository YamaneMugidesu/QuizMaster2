...
export const UserManagement: React.FC = () => {
  ...
  const [newSchoolStage, setNewSchoolStage] = useState<string[]>([]);
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    if (newSchoolStage.length === 0) {
        addToast('请至少选择一个学段', 'error');
        return;
    }
    if (newSubject.length === 0) {
        addToast('请至少选择一个学科', 'error');
        return;
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
        const updates: any = {
            ...
            providerName: editingUser.providerName,
            schoolStage: editingUser.schoolStage,
      ...
      <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 transform transition-all scale-100">
                <h3 className="text-xl font-bold text-gray-900 mb-6">添加新用户</h3>
                ...
                <form onSubmit={handleAddUser} className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2 bg-gray-50 p-2 rounded border border-gray-100">
                            {SCHOOL_STAGES.map(stage => (
                                <label key={stage} className="inline-flex items-center space-x-1 cursor-pointer hover:bg-gray-200 p-1 rounded">
                                    <input
                                        type="checkbox"
                                        value={stage}
                                        checked={newSchoolStage.includes(stage)}
                                        onChange={e => {
                                            if (e.target.checked) {
                                                setNewSchoolStage([...newSchoolStage, stage]);
                                            } else {
                                                setNewSchoolStage(newSchoolStage.filter(s => s !== stage));
                                            }
                                        }}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-5 gap-2 bg-gray-50 p-2 rounded border border-gray-100">
                            {SUBJECTS.map(subj => (
                                <label key={subj} className="inline-flex items-center space-x-1 cursor-pointer hover:bg-gray-200 p-1 rounded">
                                    <input
                                        type="checkbox"
                                        value={subj}
                                        checked={newSubject.includes(subj)}
                                        onChange={e => {
                                            if (e.target.checked) {
                                                setNewSubject([...newSubject, subj]);
                                            } else {
                                                setNewSubject(newSubject.filter(s => s !== subj));
                                            }
                                        }}
                                        className="rounded text-primary-600 focus:ring-primary-500"
                                    />
                    ...
                    <div className="flex justify-end gap-3 mt-8">
                        <Button variant="secondary" onClick={() => setIsAdding(false)} disabled={isSubmitting}>
                            取消
                        </Button>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input 
                            type="text" 
                            className="w-full px-3 py-2 border rounded-md text-sm"
                            value={editingUser.providerName || ''}
                            onChange={(e) => setEditingUser({...editingUser, providerName: e.target.value})}
                        />
                    </div>
                            ...
                            {SCHOOL_STAGES.map(stage => (
                                <label key={stage} className="inline-flex items-center space-x-1 cursor-pointer hover:bg-gray-200 p-1 rounded">
                                    <input
                                        type="checkbox"
                                        value={stage}
                                        checked={(editingUser.schoolStage || []).includes(stage)}
                                        onChange={e => {
                                            const current = editingUser.schoolStage || [];
                                            if (e.target.checked) {
                                                setEditingUser({...editingUser, schoolStage: [...current, stage]});
                                            } else {
                                                setEditingUser({...editingUser, schoolStage: current.filter(s => s !== stage)});
                                            }
                                        }}
                                        className="rounded text-primary-600 focus:ring-primary-500"
                                    />
                            ...
                            {SUBJECTS.map(subj => (
                                <label key={subj} className="inline-flex items-center space-x-1 cursor-pointer hover:bg-gray-200 p-1 rounded">
                                    <input
                                        type="checkbox"
                                        value={subj}
                                        checked={(editingUser.subject || []).includes(subj)}
                                        onChange={e => {
                                            const current = editingUser.subject || [];
                                            if (e.target.checked) {
                                                setEditingUser({...editingUser, subject: [...current, subj]});
                                            } else {
                                                setEditingUser({...editingUser, subject: current.filter(s => s !== subj)});
                                            }
                                        }}
                                        className="rounded text-primary-600 focus:ring-primary-500"
                                    />
