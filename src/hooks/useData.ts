...
const resultsFetcher = async ([_key, page, limit, filters]: [string, number, number, any]) => {
    return getPaginatedUserResults(page, limit, filters);
};
...
export const useAllResults = (
    ...
    filters: {
        ...
        status?: string;
    } | string, // Support legacy string type for backward compatibility if needed, though we will update usage
    enabled: boolean = true
) => {
    // Normalize filters if it's just a string (search term)
    const normalizedFilters = typeof filters === 'string' ? { username: filters } : filters;

    const { data, error, isLoading, mutate } = useSWR(
        enabled ? ['allResults', page, limit, normalizedFilters] : null,
        resultsFetcher,
        {
